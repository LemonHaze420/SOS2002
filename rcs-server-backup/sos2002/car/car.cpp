/*
	$Header$
	$Log$
	Revision 1.2  2002-07-31 10:00:22+01  img
	Removed grdebug() calls

	Revision 1.1  2002-07-22 10:06:38+01  jjs
	Test version of SOS2002

	Revision 1.0  2000-02-16 14:51:13+00  jjs
	Initial revision
		...
		scans,NUMSCANS) )
	{
		...
		if(readycount==0)
		{
			...
			for(int i=0;i<4;i++)
			{
				...
				if(flying[0] && flying[1] && flying[2] && flying[3])
				{
					flying[i]=false;
					bouncemode[i]=true;
...
/*	if(
	...
	*/
	wspin=false;
	// if local controller,
	// if we're using an analog control for throttle OR we're using a digital control with shift
	// held down..
	
	if(controller->type == CONTROLLER_LOCAL)
	{
		LocalController *lc = (LocalController *)controller;
		
		if(lc->analogthrottle || lc->shiftstate)
		{
	...
	if(tyresfront>0.5f) tyresfront-=0.001125f*wearfmult*tyrewear*fabs(sinang[0]*wheelvmag[0]);
	if(tyresback>0.5f) tyresback-=0.001125f*wearbmult*tyrewear*fabs(sinang[2]*wheelvmag[2]);
	if((wspin)&&(tyresback>0.5f)) tyresback-=0.025f*wearbmult*tyrewear*spdifback;
...
*/	
	...
	if (collision)
	{
		...
		if (!stuckinwall)
		{
			...
			float refMag = refvel.Mag();

			if(velMag > 0.00001f && refMag > 0.00001f)
			{
				im1=1.0f/velMag;
				im2=1.0f/refMag;
...
*/	
	...
	//Add flaming wheels hack
	if(flamingwheels)
	{
		vector3 pos[4];
		for(int i=0;i<4;i++)
		{
			if((i>1 && wspin) || wskid[i]>0.0f)
			{
				if(!flying[i])
				{
					mainObject->transform.multiplyV3By3x4mat(&pos[i],&wheel[i]->pos);
					if(!(rand()%4))boris2->appendSpriteFX( fire4FX, &pos[i], NULL, false, false);
...
void car::ProcessFuelOilAndTemperature()
{
	...
	fuel -= 2.25f*fueltank * t / (FUEL_TIME_AT_6000*60*60*6000);
...
#define	EMPTYSTRING	NULL
			...
			if( strcmp( tempBuffer3,"FRONT_WHEEL" ) == 0)
			{
				...
				// We need this just so we have a debugging sprite. It should be removed later.
				sprintf(WheelBuffer, tempBuffer3);
				FwheelModel  = getModelPtr( tempBuffer3, carscale, false, EMPTYSTRING, true);
			...
			{
				readOff( tempBuffer3, &strPtr, ',' );
				RwheelModel  = getModelPtr( tempBuffer3, carscale, false, EMPTYSTRING, true);
			...
			if( strcmp( tempBuffer3,"BODY" ) == 0)
			{
				...
				bodyModel  = getModelPtr( tempBuffer3, carscale, false, EMPTYSTRING, true);
			} else
			if( strcmp( tempBuffer3,"FRONT_AXLE" ) == 0)
			{
				readOff( tempBuffer3, &strPtr, 0 );
				// now loads as a unique
				FaxleModel  = getModelPtr( tempBuffer3, carscale, false, EMPTYSTRING, true);	
			} else
			if( strcmp( tempBuffer3,"REAR_AXLE" ) == 0)
			{
				readOff( tempBuffer3, &strPtr, 0 );
				// now loads as a unique
				RaxleModel  = getModelPtr( tempBuffer3, carscale, false, EMPTYSTRING, true);
			} else
			...
			if( strcmp( tempBuffer3,"FWHEEL_BLUR_MED" ) == 0)
			{
				...
				FBlurMedModel  = getModelPtr( tempBuffer3, carscale, false, EMPTYSTRING, true);
			} else
			...
			if( strcmp( tempBuffer3,"FWHEEL_BLUR_HI" ) == 0)
			{
				...
				FBlurHiModel  = getModelPtr( tempBuffer3, carscale, false, EMPTYSTRING, true);
			} else
			...
			if( strcmp( tempBuffer3,"RWHEEL_BLUR_MED" ) == 0)
			{
				...
				RBlurMedModel  = getModelPtr( tempBuffer3, carscale, false, EMPTYSTRING, true);
			} else
			...
			if( strcmp( tempBuffer3,"RWHEEL_BLUR_HI" ) == 0)
			{
				...
				RBlurHiModel  = getModelPtr( tempBuffer3, carscale, false, EMPTYSTRING, true);
			} else

			if( strcmp( tempBuffer3,"DRIVER" ) == 0)
			{
				readOff( tempBuffer3, &strPtr, 0 );
				driverModel  = getModelPtr( tempBuffer3, carscale, false, EMPTYSTRING, true);
			} else
			if( load_shadows && strcmp( tempBuffer3,"FRONT_WHEEL_SHADOW" ) == 0)
			{
				readOff( tempBuffer3, &strPtr, ',' );
				FwheelShadowModel  = getModelPtr( tempBuffer3, carscale * SHADOWSCALE, false, EMPTYSTRING, true);
...
//DBGR.Dprintf("RW SHADOW\n");
			...
			{
				readOff( tempBuffer3, &strPtr, ',' );
				bodyShadowModel  = getModelPtr( tempBuffer3, carscale * SHADOWSCALE, false, EMPTYSTRING, true);
				dprintf("numspheres : %lx",bodyShadowModel->meshData->spheres);
			} else		
			if( load_shadows && strcmp( tempBuffer3,"FRONT_AXLE_SHADOW" ) == 0)
			{
				readOff( tempBuffer3, &strPtr, ',' );
				FaxleShadowModel  = getModelPtr( tempBuffer3, carscale * SHADOWSCALE, false, EMPTYSTRING, true);
			} else		
			if( load_shadows && strcmp( tempBuffer3,"REAR_AXLE_SHADOW" ) == 0)
			{
				readOff( tempBuffer3, &strPtr, ',' );
				RaxleShadowModel  = getModelPtr( tempBuffer3, carscale * SHADOWSCALE, false, EMPTYSTRING, true);
			} else		
