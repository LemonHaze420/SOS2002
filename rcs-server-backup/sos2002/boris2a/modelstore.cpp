...
// $Log$
// Revision 1.1  2000-02-18 10:29:38+00  jjs
...
model * getModelPtr( char * filename, float modelscale, bool uniqueCopy, char * SSname, bool useVB )
{
	bool matched = false;
	...
	else
	{
		...
		for(numlods=0;numlods<modelChain->meshct;numlods++)
		{
			if(numlods <= modelChain->highest_visible_mesh)
				loadMulderFile( lodfilenames[numlods], modelChain->meshData+numlods, modelscale, numlods?NULL:SSname,  true ,useVB);
		}
