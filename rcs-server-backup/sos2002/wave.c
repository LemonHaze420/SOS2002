...
/*
	$Header$
	$Log$
	Revision 1.0  2000-02-21 12:34:34+00  jjs
	Initial revision

	Revision 1.3  1998/12/02 12:17:06  jcf
	Added Id properly again...

	Revision 1.2  1998/12/02 12:03:50  jcf
	Added Id properly ;)
 ...
 *      pszFileName - Input filename to load OR ptr to memory block
...
*/
int WaveOpenFile(
	char *pszFileName,                              // (IN) filename OR ptr to memory block
	HMMIO *phmmioIn,                                // (OUT)
	WAVEFORMATEX **ppwfxInfo,                       // (OUT)
	MMCKINFO *pckInRIFF,                             // (OUT)
	unsigned long mem_block_size					// NZ if this is a memory block
			)
{
#if defined(UNDER_CE)
#else
	...
	*ppwfxInfo = NULL;
	...
	else
	{
		...
		mmioinfo.pchBuffer = pszFileName;	// actually a pointer to a memory block
		mmioinfo.cchBuffer = mem_block_size;
			...
			)
{
	...
	*cbSize = 0;
	...
	if ((nError = WaveOpenFile(pszFileName, &hmmioIn, ppwfxInfo, &ckInRiff,0)) != 0)
		{
		goto ERROR_LOADING;
		}
			...
			)
{
	...
	*cbSize = 0;
	...
	if ((nError = WaveOpenFile(data, &hmmioIn, ppwfxInfo, &ckInRiff,mem_block_size)) != 0)
		{
		goto ERROR_LOADING;
		}
