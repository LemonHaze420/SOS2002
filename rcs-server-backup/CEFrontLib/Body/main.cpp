...
// $Log$
// Revision 1.55  2000-10-09 15:16:29+01  jjs
// DCSSJR07 version.
//
...
void RenderJapanText(void)
{
	if(JapanHelp>=0)
	{
		...
		static sxy japxy[]={
			...
			{280,100,346,82,0.0f,422.0f/512.0f,336.0f/512.0f,496.0f/512.0f},
			...
			{320,100,298,76,428.0f/512.0f,0.0f/512.0f,352.0f/512.0f,288.0f/512.0f},
			{290,100,283,76,508.0f/512.0f,0.0f/512.0f,432.0f/512.0f,273.0f/512.0f},
		...
		if(JapanHelp == 5 || JapanHelp == 6)
		{
			jappoly[0].tu = jappoly[1].tu = japxy[JapanHelp].u1;
			jappoly[0].tv = jappoly[3].tv = japxy[JapanHelp].v1;
			jappoly[2].tu = jappoly[3].tu = japxy[JapanHelp].u2;
			jappoly[1].tv = jappoly[2].tv = japxy[JapanHelp].v2;
		}
		else
		{
...
{
	MainGame = NULL;
	hNewDeviceEvent=NULL;
	hDeviceGoneEvent=NULL;
	boris2camera = NULL;
	boris2display = NULL;
}
...
bool ConsoleFrontLib::Start()
{
	...
	boris2camera  = new camera( &vector3(0,1000.0f,0),
								600.0f, 3200 ); // zeye 0 = 360 900 = 90 e.t.c.

	boris2display  = new display(640, 480, 0,0);	
	...
	static __int64 PerfCounter, PerfFrequency;
