...
bool Game::Load(int ReEntryPoint)
{
...
void Game::Destroy()
{
	...
	for (int j=0 ; j<3 ; j++)
		for (i=0 ; i<15 ; i++)
		{
			if (AllScenarios[j][i])
			{
				safe_free(AllScenarios[j][i]->Name);
				safe_free(AllScenarios[j][i]->Description);
				safe_free(AllScenarios[j][i]->Track);
				safe_free(AllScenarios[j][i]->LengthString);
				safe_free(AllScenarios[j][i]->SuccessString);
				safe_free(AllScenarios[j][i]->FailureString);
				safe_delete( AllScenarios[j][i]);
			}
			if (ChampionshipScenarios[j][i])
			{
				safe_free(ChampionshipScenarios[j][i]->Name);
				safe_free(ChampionshipScenarios[j][i]->Description);
				safe_free(ChampionshipScenarios[j][i]->LengthString);
				safe_free(ChampionshipScenarios[j][i]->SuccessString);
				safe_free(ChampionshipScenarios[j][i]->FailureString);
				safe_delete( ChampionshipScenarios[j][i]);
			}
		}

	
	for (i=0 ; i<7 ; i++) safe_delete( RaceOptionsBank[i]);
	for (i=0 ; i<7 ; i++) safe_delete( ChampionshipBank[i]);
	for (i=0 ; i<3 ; i++) safe_delete( SingleRaceABank[i]);
	for (i=0 ; i<2 ; i++) safe_delete( VMOptionsBank[i]);
	for (i=0 ; i<4 ; i++) safe_delete( ScenarioBank[i]);

	safe_delete( MainMenu);

	safe_delete( DefaultAlphabet);
	safe_delete( YellowAlphabet);

	for (i=0 ; i<15 ; i++) 
		safe_delete( AllCars[i]);

	safe_delete( _SlideBar);
	safe_delete( _SlideKnob);

	for (i=0 ; i<15 ; i++)
	{
		safe_delete( CarGridModels[i]);
		safe_delete( CarTableModels[i]);
	}


	safe_delete( _MemoryCardOne);
	safe_delete( _MemoryCardTwo);
	safe_delete( _MemoryCardBlack);
	safe_delete( GlobalMemoryCard);

	for (i=0 ; i<9 ; i++)
	{
		safe_delete( TrackModels[i]);
		safe_delete( TrackStatsModels[i]);
		safe_delete( TrackNameModels[i]);
	}

	safe_delete( GlobalTrack);

	safe_delete( LeftArrow);
	safe_delete( RightArrow);
	safe_delete( UpArrow);
	safe_delete( DownArrow);

	safe_delete( EndLetter);
	safe_delete( DelLetter);
	safe_delete( Cursor);
	safe_delete( Space);
	...
	for (i=0 ; i<2 ; i++)
		for (int j=0 ; j<3 ; j++)
			safe_delete( SoundObjects[i][j]);

	for (i=HEADER_RACEOPTIONS ; i<=HEADER_SINGLERACE ; i++)
		safe_delete( ScreenHeaders[i]);
