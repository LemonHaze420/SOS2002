...
// $Log$
// Revision 1.16  2000-05-08 19:53:23+01  img
// Final? build
//
...
OptionsScreen::OptionsScreen(Alphabet *DefaultAlphabet)
{
	...
	Stereo = new TextWord("STEREO", MainAlphabet);
	Mono = new TextWord("MONO", MainAlphabet);

	if(smode)
		Mono->Write(400,330,1200.0f);
	else
		Stereo->Write(400,330,1200.0f);
...
void OptionsScreen::Highlight()
{
	...
	{
		if(smode)
			Mono->Write(400,330,1000.0f);
		else
			Stereo->Write(400,330,1000.0f);
		LeftArrow->Draw(330,333,600.0f);
		RightArrow->Draw(470,333,600.0f);
	}
}
...
int OptionsScreen::ControlPressed(int ControlPacket)
{
	...
	{
		Lowlight();
		if (--CurrentObject < 0) CurrentObject = SO_EFFECTS + 1;
		if(CurrentObject <=SO_EFFECTS)
	...
	{
		Lowlight();
		if (++CurrentObject > SO_EFFECTS + 1) CurrentObject = 0;
		if(CurrentObject <=SO_EFFECTS)
			Objects[CurrentObject]->Rotate();
		Highlight();
		Counter=0;
...
{
	SetFirmwareValues(DWORD_DONT_CHANGE , BYTE_DONT_CHANGE,BYTE_DONT_CHANGE,smode,BYTE_DONT_CHANGE);
	Lowlight();
