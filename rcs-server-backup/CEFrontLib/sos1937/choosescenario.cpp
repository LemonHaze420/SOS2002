...
// $Log$
// Revision 1.23  2000-10-09 15:16:32+01  jjs
// DCSSJR07 version.
//
// Revision 1.22  2000-05-08 19:53:14+01  img
// Final? build
//
...
{
	char Filename[256], Buffer2[2048], CarNumber[10], *Pointer=Buffer2, *Buffer = Buffer2, Data[1024];
	...
	for (int j=0 ; j<3 ; j++)
	{
		for (int i=0; i<15 ; i++)
		{
			...
			for(ii=0;ii<strlen(Filename);++ii)
				fbuf[ii]=Filename[ii];
			fbuf[ii]=0;
			if((CurrentFile=CreateFile(fbuf,GENERIC_READ,0,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL)) != INVALID_HANDLE_VALUE)
			{
				DWORD bytesRead;
				AllScenarios[j][i] = new Scenario();

				memset(AllScenarios[j][i],0,sizeof(Scenario));

				
				ReadFile(CurrentFile,Buffer,2048,&bytesRead,NULL);
				AllScenarios[j][i]->GridPosition=-1;
				READFIELD("Name",Name);
#if defined(JAPAN)
				Pointer = Buffer;
				while (*Pointer && (strnicmp( "Description" , Pointer, strlen( "Description" )) || *(Pointer+strlen( "Description" )) != 32)) 
				if ((int)(Pointer = strchr(Pointer, '\n')+1) == 1) 
					break;
				if ((int)Pointer > 1) 
				{ 
					Pointer += strlen( "Description" );
					while (*Pointer!='\n') 
						Pointer++;
					Pointer++;
					char *tPointer = Pointer;
					for(int counter = 5;counter--;)
					{
						while(*tPointer++!='\n');
					}
					strncpy(Data , Pointer, Length = tPointer - Pointer);
					Data[Length] = 0;
					Pointer+=Length;
					AllScenarios[j][i]->Description = strdup(Data);
				}
				Buffer = Pointer;
#else
	...
	for (j=0 ; j<3 ; j++)
	{
		for (int i=0; i<15 ; i++)
		{
			...
			if((CurrentFile=CreateFile(fbuf,GENERIC_READ,0,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL)) != INVALID_HANDLE_VALUE)
			{
				...
				memset(ChampionshipScenarios[j][i],0,sizeof(Scenario));
				...
				{ 
					...
					Data[Length] = 0;
					Pointer+=Length;
					ChampionshipScenarios[j][i]->Description = strdup(Data);
				}
				Buffer = Pointer;	// Skip the first bits as they user race in the description.
				...
				ChampionshipScenarios[j][i] = NULL;
		}
	}
